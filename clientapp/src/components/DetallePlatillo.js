...
function DetallePlatillo() {
  ...
  const { ordensesionindividual, obtenerOrdenSesionIndividual, agregarOrden } =
    ordensContext;
  ...
  const crearPlatilloOrdenado = (e) => {
    ...
    platilloOrdenadoAux.sesionIndividual =
      localStorage.getItem("sesionindividualid");
  ...
  return (
    <Container fluid className="platillo-individual">
      <Row>
      ...
      <Row>
        <Form>
          <Form.Group className="input-notas ml-4">
            <textarea
              className="input-notas-texto form-control"
              rows="3"
      ...
      <Row>
        <Col className="boton-ordenar">
          ...
          <Button
            className="confirmar"
            onClick={() => guardarPlatilloEnOrdenLocal()}
          >
            Agregar
          </Button>
          {/*</Link>*/}
